...
def masks_to_boxes(masks):
    ...
    h, w = masks.shape[-2:]  # torch.Size([1, 1024, 1024])  0~255
    ...
    x_mask = ((masks>128) * x.unsqueeze(0))  # torch.Size([1, 1024, 1024])
    x_max = x_mask.flatten(1).max(-1)[0]  # torch.Size([1])
    x_min = x_mask.masked_fill(~(masks>128), 1e8).flatten(1).min(-1)[0]  # torch.Size([1])

    y_mask = ((masks>128) * y.unsqueeze(0))  # y_mask torch.Size([1, 1024, 1024])
...
def box_noise(boxes, box_noise_scale=0.2):
